...
final class User extends BaseController{
    ...
    public function index(){
        ...
        if(isset($_GET['name']) && !empty($_GET['name'])){
            ...
            $where = "class LIKE '%{$_GET['class']}%'";
            $parms = array("class"=>$_GET['class']);
            $mode  = "class";
        }else{
            $where = "2>1";
            $parms = array();
    ...
    public function insert(){
        ...
        if($usermodel->rowCount("id={$user['id']}")){
            $this->sendJsonMessage("The user ID already exists",1);
        }

        if($usermodel->insert($user)){
            $this->sendJsonMessage("Add user success",0);
        }else{
            $this->sendJsonMessage("Add user failed",1);
        }
    }
    ...
    public function open(){
        ...
        if($userModel->update(array("status"=>1),"id={$id}")){
            $this->sendJsonMessage("Enabled success",0);
        }else{
            $this->sendJsonMessage("Enabled failed",1);
        }
    }
    ...
    public function changePwd(){
        ...
        if(!$_POST['pwd']){
            $this->sendJsonMessage("Please enter your password",1);
        }
