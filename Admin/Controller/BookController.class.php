...
final class Book extends BaseController{
    ...
    public function index(){
        ...
        $eachPerPage = 15;      //每页显示条数

        //获取搜索条件
        if(isset($_GET['keyword']) && !empty($_GET['keyword'])){
            $where = "author LIKE '%{$_GET['keyword']}%'";
            $parms = array("keyword"=>$_GET['keyword']);
            $mode  = "keyword";
        }else if(isset($_GET['keyworda']) && !empty($_GET['keyworda'])){
            $where = "name LIKE '%{$_GET['keyworda']}%'";
            $parms = array("keyworda"=>$_GET['keyworda']);
            $mode  = "keyworda";
		}else if(isset($_GET['keywordp']) && !empty($_GET['keywordp'])){
            $where = "press LIKE '%{$_GET['keywordp']}%'";
            $parms = array("keywordp"=>$_GET['keywordp']);
            $mode  = "keywordp";
		}else if(isset($_GET['keywordi']) && !empty($_GET['keywordi'])){
            $where = "isbn LIKE '%{$_GET['keywordi']}%'";
            $parms = array("keywordi"=>$_GET['keywordi']);
            $mode  = "keywordi";
    ...
    public function insert(){
        ...
        if(in_array("",$bookInfo)){
            $this->sendJsonMessage("Please enter full information",1);
        }
        ...
        if($bookModel->insert($bookInfo)){
            ...
            $this->sendJsonMessage("Add failed",1);
        }
    }
    ...
    public function update(){
        ...
        if($bookModel->update($bookInfo,"id={$id}")){
            $this->sendJsonMessage("Modified success",0);
        }else{
            $this->sendJsonMessage("Modified failed",1);
        }
    }
    ...
    public function delete(){
        ...
        if($bookModel->delete("id={$id}") && $borrowModel->delete("book_id={$id}")){
            $this->sendJsonMessage("Delete success",0);
        }else{
